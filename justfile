...
export UBUNTU_TAG := env_var_or_default('UBUNTU_TAG','noble-20250619')
export JAVA_VER_DISTRO_8 := env_var_or_default('JAVA_VER_DISTRO_8','8.0.452-zulu')
export JAVA_VER_DISTRO_11 := env_var_or_default('JAVA_VER_DISTRO_11','11.0.27-zulu')
export JAVA_VER_DISTRO_17 := env_var_or_default('JAVA_VER_DISTRO_17','17.0.15-zulu')
export JAVA_VER_DISTRO_21 := env_var_or_default('JAVA_VER_DISTRO_21','21.0.7-zulu')
export JAVA_VER_DISTRO_24 := env_var_or_default('JAVA_VER_DISTRO_24','24.0.1-zulu')
...
export GRADLE_VER := env_var_or_default('GRADLE_VER','8.14.2')
export MAVEN_VER := env_var_or_default('MAVEN_VER','3.9.10')
...
export JENA_MAIN_GIT_COMMIT_ID := env_var_or_default('JENA_MAIN_GIT_COMMIT_ID','85b9f665')
export JENA_MAIN_DISTRO_VERSION := env_var_or_default('JENA_MAIN_DISTRO_VERSION','5.5.0-SNAPSHOT')
...
# Apache Jena recipes
build-jena: build-jena-main-17 build-jena-main-21 build-jena-release-5_4
...
build-jena-release-5_4: build-maven-17
   #!/usr/bin/env bash
   IMGTAG={{prefix}}ubuntu-jena:${JENA_RELEASE_5_4_DISTRO_VERSION}
   if [[ "{{do_platform_amd64}}" == "true" ]]; then _PLATFORMS+=("linux/amd64"); fi
   if [[ "{{do_platform_arm64}}" == "true" ]]; then _PLATFORMS+=("linux/arm64"); fi
   if [[ "{{use_cache}}" == "true" ]]; then
      CACHE=" --cache-from type=local,src=$(pwd)/{{external_cache_dir_name}}/jena/jena-release-5_4 --cache-to type=local,dest=$(pwd)/{{external_cache_dir_name}}/jena/jena-release-5_4 "
   fi
   for I in ${!_PLATFORMS[@]}; do
      if [[ ${I} -gt 0 ]]; then PLATFORMS="${PLATFORMS},"; fi
      PLATFORMS="${PLATFORMS}${_PLATFORMS[$I]}"
   ...
   if [[ "${PLATFORMS}" != "" ]]; then
      time docker image build -f Dockerfile.ubuntu-jena -t ${IMGTAG} \
                              ...
                              --progress plain \
                              --build-arg PREFIX={{prefix}} \
                              --build-arg PARENT_TAG=${JENA_RELEASE_5_4_PARENT_TAG} \
                              --build-arg JENA_GIT_COMMIT_ID=${JENA_RELEASE_5_4_GIT_COMMIT_ID} \
                              --build-arg JENA_DISTRO_VERSION=${JENA_RELEASE_5_4_DISTRO_VERSION} \
                              .
   fi
   just _push_image "${IMGTAG}" {{post_push_sleep_seconds}}
