...
export UBUNTU_TAG := env_var_or_default('UBUNTU_TAG','noble-20251001')
export JAVA_VER_DISTRO_8 := env_var_or_default('JAVA_VER_DISTRO_8','8.0.472-zulu')
export JAVA_VER_DISTRO_11 := env_var_or_default('JAVA_VER_DISTRO_11','11.0.29-zulu')
export JAVA_VER_DISTRO_17 := env_var_or_default('JAVA_VER_DISTRO_17','17.0.17-zulu')
export JAVA_VER_DISTRO_21 := env_var_or_default('JAVA_VER_DISTRO_21','21.0.9-zulu')
export JAVA_VER_DISTRO_25 := env_var_or_default('JAVA_VER_DISTRO_25','25.0.1-zulu')
export JBANG_VER := env_var_or_default('JBANG_VER', '0.132.1')
export KOTLIN_VER := env_var_or_default('KOTLIN_VER','2.2.20')
export SCALA_VER := env_var_or_default('SCALA_VER','3.7.3')
export ANT_VER := env_var_or_default('ANT_VER','1.10.14')
export GRADLE_VER := env_var_or_default('GRADLE_VER','9.1.0')
export MAVEN_VER := env_var_or_default('MAVEN_VER','3.9.11')
export SBT_VER := env_var_or_default('SBT_VER','1.11.7')
...
export CASSANDRA_TRUNK_GIT_COMMIT_ID := env_var_or_default('CASSANDRA_TRUNK_GIT_COMMIT_ID','accdaaeb')
...
export CASSANDRA_RELEASE_5_0_GIT_COMMIT_ID := env_var_or_default('CASSANDRA_RELEASE_5_0_GIT_COMMIT_ID','cassandra-5.0.5')
export CASSANDRA_RELEASE_5_0_DISTRO_VERSION := env_var_or_default('CASSANDRA_RELEASE_5_0_DISTRO_VERSION','5.0.5')
...
export CASSANDRA_RELEASE_4_1_GIT_COMMIT_ID := env_var_or_default('CASSANDRA_RELEASE_4_1_GIT_COMMIT_ID','cassandra-4.1.10')
export CASSANDRA_RELEASE_4_1_DISTRO_VERSION := env_var_or_default('CASSANDRA_RELEASE_4_1_DISTRO_VERSION','4.1.10')
export JENA_MAIN_GIT_COMMIT_ID := env_var_or_default('JENA_MAIN_GIT_COMMIT_ID','b7f77747')
export JENA_MAIN_DISTRO_VERSION := env_var_or_default('JENA_MAIN_DISTRO_VERSION','5.7.0-SNAPSHOT')
export JENA_RELEASE_5_5_PARENT_TAG := env_var_or_default('JENA_RELEASE_5_5_PARENT_TAG','17')
export JENA_RELEASE_5_5_GIT_COMMIT_ID := env_var_or_default('JENA_RELEASE_5_5_GIT_COMMIT_ID','jena-5.5.0')
export JENA_RELEASE_5_5_DISTRO_VERSION := env_var_or_default('JENA_RELEASE_5_5_DISTRO_VERSION','5.5.0')
export SPARK_MASTER_GIT_COMMIT_ID := env_var_or_default('SPARK_MASTER_GIT_COMMIT_ID','8915c607')
...
# Apache Jena recipes
build-jena: build-jena-main-17 build-jena-main-21 build-jena-release-5_5
...
build-jena-release-5_5: build-maven-17
   #!/usr/bin/env bash
   IMGTAG={{prefix}}ubuntu-jena:${JENA_RELEASE_5_5_DISTRO_VERSION}
   if [[ "{{do_platform_amd64}}" == "true" ]]; then _PLATFORMS+=("linux/amd64"); fi
   if [[ "{{do_platform_arm64}}" == "true" ]]; then _PLATFORMS+=("linux/arm64"); fi
   if [[ "{{use_cache}}" == "true" ]]; then
   ...
   if [[ "${PLATFORMS}" != "" ]]; then
      time docker image build -f Dockerfile.ubuntu-jena -t ${IMGTAG} \
                              ...
                              --progress plain \
                              --build-arg PREFIX={{prefix}} \
                              --build-arg PARENT_TAG=${JENA_RELEASE_5_5_PARENT_TAG} \
                              --build-arg JENA_GIT_COMMIT_ID=${JENA_RELEASE_5_5_GIT_COMMIT_ID} \
                              --build-arg JENA_DISTRO_VERSION=${JENA_RELEASE_5_5_DISTRO_VERSION} \
                              .
   fi
   just _push_image "${IMGTAG}" {{post_push_sleep_seconds}}
